...
void Multiplication(vector<unsigned int>& result, vector<unsigned int>& A, vector<unsigned int>& B, vector<unsigned int>& carry, unsigned int taille)
{
	unsigned int k = 0;
...
void multiplication(vector<vector<unsigned int> >& result, vector<vector<unsigned int> >& A, vector<vector<unsigned int> >& B, unsigned int taille)
{
	...
	Multiplication(tempResultat, A[0], B[0], carry, j);
	...
	for (unsigned int k = 1; k < A.size() + B.size() - 1; k++)
	{
		for (unsigned int i = 0; i <= k ; i++)
		{
			if ((i < A.size()) && (k - i < B.size()))
			{
				...
				Multiplication(tempProduit, A[i], B[k - i], carry, 2 * j);
